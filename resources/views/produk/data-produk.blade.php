        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <div class="flex items-center space-x-3 self-end sm:self-auto">
                ...
                <form method="GET" action="{{ route('data-produk') }}" class="relative">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari Produk" class="w-64 pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition placeholder-gray-400"/>
                </form>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100 text-sm text-gray-600">
                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        <td class="py-4 px-4 font-medium text-gray-500">{{ $produk->firstItem() + $index }}</td>
                        <td class="py-4 px-4 font-medium text-slate-700">{{ $item->nm_produk }}</td>
                        <td class="py-4 px-4 text-slate-500">{{ $item->kategori->nm_kategori ?? 'Tanpa Kategori' }}</td>
                        <td class="py-4 px-4 font-medium text-gray-700">Rp {{ number_format($item->harga, 0, ',', '.') }}</td>
                        <td class="py-4 px-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <a href="{{ route('produk.edit', $item) }}" class="p-2 rounded-lg bg-[#E2F5E9] text-[#28A745] hover:bg-[#28A745] hover:text-white transition duration-200 cursor-pointer" title="Edit">
                                ...
                                <form method="POST" action="{{ route('produk.destroy', $item) }}" onsubmit="return confirm('Yakin hapus produk ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="py-8 text-center text-gray-400">Data produk tidak ditemukan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
